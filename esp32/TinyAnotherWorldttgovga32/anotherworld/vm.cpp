...
{
 unsigned char variableId = _scriptPtr.fetchByte(); //JJ uint8_t variableId = _scriptPtr.fetchByte();
 short int value = _scriptPtr.fetchWord(); //JJ int16_t value = _scriptPtr.fetchWord();
...
{
 unsigned char dstVariableId = _scriptPtr.fetchByte(); //JJ uint8_t dstVariableId = _scriptPtr.fetchByte();
 unsigned char srcVariableId = _scriptPtr.fetchByte(); //JJ uint8_t srcVariableId = _scriptPtr.fetchByte();	
...
{
 unsigned short int offset = _scriptPtr.fetchWord(); //JJ uint16_t offset = _scriptPtr.fetchWord();
 unsigned char sp = _stackPtr; //JJ uint8_t sp = _stackPtr;
...
{
 unsigned short int pcOffset = _scriptPtr.fetchWord(); //JJ uint16_t pcOffset = _scriptPtr.fetchWord();
...
{
 unsigned char threadId = _scriptPtr.fetchByte(); //JJ uint8_t threadId = _scriptPtr.fetchByte();
 unsigned short int pcOffsetRequested = _scriptPtr.fetchWord(); //JJ uint16_t pcOffsetRequested = _scriptPtr.fetchWord();
...
{
 unsigned char i = _scriptPtr.fetchByte(); //JJ uint8_t i = _scriptPtr.fetchByte();
...
void VirtualMachine::op_blitFramebuffer() {
  ...
  if ((timeToSleep > 0) && (gb_use_speed_fast==0))
  {
  ...
  lastTimeStamp = sys->getTimeStamp();
    ...
    if (gb_use_speed_fast==0)
    {
     video->updateDisplay(pageId);
    }
    else
    {
     unsigned int aux_time_vga= millis(); //JJ SDL_GetTicks();
     if ((aux_time_vga-gb_real_vga_time_ini)>=19)
     {
      gb_real_vga_time_ini= aux_time_vga;
      video->updateDisplay(pageId);
     }
    }
	...
	unsigned int aux_time_fps= millis(); //JJ SDL_GetTicks();
...
void VirtualMachine::snd_playSound(unsigned short int resNum, unsigned char freq, unsigned char vol, unsigned char channel) 
{
	...
	else 
	{
		...
		if (!(freq < 40))
		{
         Serial.printf("ERROR snd_playSound\r\n");
         //fflush(stdout);
         return;
        }
...
void VirtualMachine::snd_playMusic(uint16_t resNum, uint16_t delay, uint8_t pos) {
	...
	//JJdebug(DBG_SND, "snd_playMusic(0x%X, %d, %d)", resNum, delay, pos);
	//printf("snd_playMusic(0x%X, %d, %d)\n", resNum, delay, pos);
	//fflush(stdout);
