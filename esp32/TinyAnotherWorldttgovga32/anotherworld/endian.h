 ...
 */
 ...
 //JJ #include <stdint.h> //No la necesito
 ...
 inline unsigned short int READ_BE_UINT16(const void *ptr)
 {
  const unsigned char *b = (const unsigned char *)ptr; //JJ const uint8_t *b = (const uint8_t *)ptr;
  return (b[0] << 8) | b[1];
 }

 //inline uint32_t READ_BE_UINT32(const void *ptr) 
 inline unsigned int READ_BE_UINT32(const void *ptr) 
 {
  const unsigned char *b = (const unsigned char *)ptr;	//JJ const uint8_t *b = (const uint8_t *)ptr;
 ...
 //JJ inline uint16_t READ_BE_UINT16(const void *ptr) 
 inline unsigned short int READ_BE_UINT16(const void *ptr) 
 {
  return *(const unsigned short int *)ptr; //JJ return *(const uint16_t *)ptr;
 }

 //JJ inline uint32_t READ_BE_UINT32(const void *ptr) 
 inline unsigned int READ_BE_UINT32(const void *ptr) 
 {
  return *(const unsigned int *)ptr;  //JJ return *(const uint32_t *)ptr;
 }
