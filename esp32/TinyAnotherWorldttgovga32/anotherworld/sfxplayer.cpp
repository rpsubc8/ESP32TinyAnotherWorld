...
//JJ void SfxPlayer::setEventsDelay(uint16_t delay) 
void SfxPlayer::setEventsDelay(unsigned short int delay)
{
...
//JJ void SfxPlayer::loadSfxModule(uint16_t resNum, uint16_t delay, uint8_t pos) 
void SfxPlayer::loadSfxModule(unsigned short int resNum, unsigned short int delay, unsigned char pos) 
{
...
//JJ void SfxPlayer::prepareInstruments(const uint8_t *p) 
void SfxPlayer::prepareInstruments(const unsigned char *p) 
{
 ...
 for (int i = 0; i < 15; ++i) 
 {
  ...
  unsigned short int resNum = READ_BE_UINT16(p); p += 2; //JJ uint16_t resNum = READ_BE_UINT16(p); p += 2;
  if (resNum != 0) 
  {
...
{
 MutexStack(sys, _mutex);
 unsigned char order = _sfxMod.orderTable[_sfxMod.curOrder]; //JJ uint8_t order = _sfxMod.orderTable[_sfxMod.curOrder];
 const unsigned char *patternData = _sfxMod.data + _sfxMod.curPos + order * 1024; //JJ const uint8_t *patternData = _sfxMod.data + _sfxMod.curPos + order * 1024;
 //JJ for (uint8_t ch = 0; ch < 4; ++ch) 
...
//JJ void SfxPlayer::handlePattern(uint8_t channel, const uint8_t *data) 
void SfxPlayer::handlePattern(unsigned char channel, const unsigned char *data) 
{
 SfxPattern pat;
 ...
 {
  unsigned short int sample = (pat.note_2 & 0xF000) >> 12; //JJ uint16_t sample = (pat.note_2 & 0xF000) >> 12;
  if (sample != 0) 
  {
   unsigned char *ptr = _sfxMod.samples[sample - 1].data; //JJ uint8_t *ptr = _sfxMod.samples[sample - 1].data;
   if (ptr != 0) 
   {
	...
	unsigned short int loopLen = READ_BE_UINT16(ptr + 2) * 2; //JJ uint16_t loopLen = READ_BE_UINT16(ptr + 2) * 2;
	if (loopLen != 0) 
    {
	...
	short int m = pat.sampleVolume; //JJ int16_t m = pat.sampleVolume;
	unsigned char effect = (pat.note_2 & 0x0F00) >> 8; //JJ uint8_t effect = (pat.note_2 & 0x0F00) >> 8;
	if (effect == 5) 
    { // volume up
	 unsigned char volume = (pat.note_2 & 0xFF); //JJ uint8_t volume = (pat.note_2 & 0xFF);
	 m += volume;
	 if (m > 0x3F) 
     {
	  m = 0x3F;
  ...
  if (pat.note_1 != 0) 
  {
   ...
   else
   {
    if (pat.sampleBuffer != 0) 
    {
	 ...
	 //JJ assert(pat.note_1 >= 0x37 && pat.note_1 < 0x1000);
	 if (!(pat.note_1 >= 0x37 && pat.note_1 < 0x1000))
	 {
      Serial.printf("ERROR SfxPlayer handlePattern\r\n");
      //fflush(stdout);
      return;
     }
	 // convert amiga period value to hz
	 unsigned short int freq = 7159092 / (pat.note_1 * 2); //JJ uint16_t freq = 7159092 / (pat.note_1 * 2);
...
//JJ uint32_t SfxPlayer::eventsCallback(uint32_t interval, void *param) 
unsigned int SfxPlayer::eventsCallback(unsigned int interval, void *param) 
{
...
// {
//  unsigned short int delay = _delay; //JJ uint16_t delay = _delay;
